...
import { getMissionSideRoleLabels, missionObjectiveLabels } from '@/entities/mission/lib';
...
export const MissionDetails: FC<MissionDetailsProps> = ({ game, attackSideType, defenseSideType }) => {
  ...
  } = resolveUniformScreenshots(game.missionVersion);
  ...
  const friendlyColor = game.missionVersion.friendlySideType
    ? resolveMissionSideColor(game.missionVersion.friendlySideType)
    : null;
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          {game.mission.missionObjective && (
            <span className="shrink-0 rounded border border-lime-500/40 bg-lime-950/40 px-2 py-1 text-xs font-semibold text-lime-200">
              {missionObjectiveLabels[game.mission.missionObjective]}
            </span>
          )}
        </div>
