...
export type ConfirmSignUpErrorMessage =
  | (typeof CONFIRM_SIGN_UP_MESSAGES)[keyof typeof CONFIRM_SIGN_UP_MESSAGES]
  | 'unknown';

export class ConfirmSignUpState {
  constructor() {
  ...
  confirmSignUp = async (token: string) => {
    try {
      ...
      await authApi.confirmSignUp(token);
...
const confirmSignUpState = new ConfirmSignUpState();

export { confirmSignUpState };
