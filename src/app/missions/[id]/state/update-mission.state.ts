...
import { Mission, MissionCommentMessage, MissionObjective, MissionType, UpdateMissionDto } from '@/shared/sdk/types';
...
export class UpdateMissionState {
  constructor() {
  ...
  async save(data: MissionFormData, imageFile: File | null, canUpdateCoauthors = true, onSuccess?: () => void) {
    try {
      ...
      await missionsApi.updateMission(dto);
