...
import { PlanManagementSections } from './plan-management-sections';
...
export const HqPlans = observer(({ activePlanId }: HqPlansProps) => {
  ...
  const model = useMemo(() => new HqPlansState(), []);
  const socketRef = useRef<Socket | null>(null);
  const selectedPlanIdRef = useRef<string | undefined>(activePlanId);
  const joinedPlanIdsRef = useRef<string[]>([]);
  const deleteHqCommentModel = useMemo(() => new DeleteMissionCommentState(), []);

  useEffect(() => {
    selectedPlanIdRef.current = activePlanId;
  }, [activePlanId]);

  const currentUser = session.user?.data;
  const hasAccess = Boolean(
    currentUser?.squad &&
      (currentUser.squad.side?.type === SideType.BLUE || currentUser.squad.side?.type === SideType.RED),
  );
  const currentSide = currentUser?.squad?.side?.type;
  const currentSquad = currentUser?.squad;
  const isHqAdmin = UserModel.hasAnyRole(currentUser?.roles, [
    UserRole.OWNER,
