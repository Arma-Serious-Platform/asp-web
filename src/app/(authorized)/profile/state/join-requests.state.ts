...
export class SquadJoinRequestsState {
  constructor() {
  ...
  load = async () => {
    try {
      this.loader.start();
      const { data } = await squadsApi.squadJoinRequests();
      this.requests = data;
    } catch (error: any) {
  ...
  acceptRequest = async (requestId: string, onSuccess?: (request: SquadJoinRequest) => void | Promise<void>) => {
    try {
      ...
      const { data } = await squadsApi.acceptSquadJoinRequest(requestId);
  ...
  rejectRequest = async (requestId: string, onSuccess?: (request: SquadJoinRequest) => void | Promise<void>) => {
    try {
      this.actionLoader.start();
      const { data } = await squadsApi.rejectSquadJoinRequest(requestId);
