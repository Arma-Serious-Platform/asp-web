...
import { UserHistoryState } from '../state/user-history.state';
...
const describeEvent = (event: UserHistoryEvent) => {
  ...
  switch (event.type) {
    ...
    case UserHistoryEventType.ROLE_CHANGE: {
      ...
      return `${UserModel.getRoleText(oldRoles)} → ${UserModel.getRoleText(newRoles)}`;
    }
...
export const UserHistorySection: FC<{ userId?: string }> = observer(({ userId }) => {
  const model = useMemo(() => new UserHistoryState(), []);
