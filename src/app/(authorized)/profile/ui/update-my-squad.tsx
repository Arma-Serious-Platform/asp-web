...
import { SocialKeys } from '@/features/user/change-socials/data';
...
export const UpdateMySquadForm: FC<UpdateMySquadFormProps> = ({ squad, onUpdated }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setIsSaving(true);
      const { data } = await squadsApi.updateMySquad(dto);
  ...
  const handleChangeSocials = async (changes: Partial<Record<SocialKeys, string>>) => {
    try {
      setIsSaving(true);
      const { data } = await squadsApi.updateMySquad(changes);
  ...
  const handleChangeDescription = async ({ lexicalState }: MessageEditorSubmitPayload) => {
    try {
      setIsSaving(true);
      const { data } = await squadsApi.updateMySquad({ description: lexicalState });
