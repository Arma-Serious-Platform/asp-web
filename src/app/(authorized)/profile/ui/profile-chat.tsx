...
import { chatApi, usersApi } from '@/shared/sdk';
...
import { MessageAttachmentItem, normalizeMessageAttachments } from '@/entities/attachment/ui/attachment';
...
export const ProfileChat = observer(function ProfileChat({ initialUserId, onInitialUserHandled }: ProfileChatProps) {
  ...
  const loadMessages = useCallback(
    async (chatId: string) => {
      ...
      try {
        const { data } = await chatApi.findChatMessages(chatId);
  ...
  const handleUpdateMessage = useCallback(
    async (message: ChatMessage, payload: MessageComposerSubmitPayload) => {
      ...
      setIsSending(true);
      try {
        await chatApi.updateChatMessage(activeChat.id, message.id, {
  ...
  const confirmDeleteMessage = useCallback(async () => {
    ...
    try {
      await chatApi.deleteChatMessage(activeChat.id, messageToDelete.id);
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      const { data } = await usersApi.findUsers({ take: 1000, skip: 0 });
    ...
    async (items: Chat[]) => {
      ...
      const details = await Promise.all(
        items.map(async chat => {
          try {
            const { data } = await chatApi.findChatById(chat.id);
  ...
  const saveChatRename = useCallback(async () => {
    ...
    try {
      const { data } = await chatApi.updateChat(activeChatId, { name: trimmedName });
  ...
  const handleLeaveChat = useCallback(async () => {
    ...
    try {
      await chatApi.leaveChat(activeChatId);
  ...
  const loadActiveChatParticipants = useCallback(async (chatId: string) => {
    try {
      const { data } = await chatApi.findChatById(chatId);
  ...
  const addSelectedMembers = useCallback(async () => {
    ...
    try {
      await chatApi.addChatMembers(activeChatId, { userIds: selectedMemberIds });
  ...
  useEffect(() => {
    const loadChats = async () => {
      ...
      try {
        const { data } = await chatApi.findChats();
    ...
    const openDirectChat = async () => {
      try {
        ...
        if (target) {
          ...
          const { data } = await usersApi.getUserByIdOrNickname(initialUserId);
                ...
                onSubmit={async ({ lexicalState, attachments }) => {
                  ...
                  try {
                    if (attachments.length > 0) {
                      await chatApi.sendChatMessage(activeChat.id, {
                    ...
                    if (socket?.connected) {
                      ...
                      await chatApi.sendChatMessage(activeChat.id, { content: lexicalState });
                      await loadMessages(activeChat.id);
                    }
                  } catch (error) {
                    console.error(error);
