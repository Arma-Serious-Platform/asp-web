...
import { specializationsApi, squadsApi } from '@/shared/sdk';
import { InviteToSquadModal } from './invite-to-squad';
import { inviteToSquadState } from '../state/invite-to-squad.state';
import { View } from '@/features/view';
import { SquadInviteList } from './accept-or-reject-invite';
import { acceptOrRejectInviteState } from '../state/accept-or-reject-invite.state';
import { Avatar } from '@/shared/ui/organisms/avatar';
import { UserNicknameText } from '@/entities/user/ui/user-text';
import { KickFromSquadModal } from './kick-from-squad';
import { kickFromSquadState } from '../state/kick-from-squad.state';
import { LeaveFromSquadModal } from './leave-from-squad';
import { leaveFromSquadState } from '../state/leave-from-squad.state';
import { UpdateMySquadForm } from './update-my-squad';
import { SquadJoinRequestsState } from '../state/join-requests.state';
import { SquadJoinRequestList } from './join-requests';
...
export const UserSquad: FC<{
  ...
  const joinRequestsModel = useMemo(() => new SquadJoinRequestsState(), []);
  ...
  const squadModel = useMemo(() => (squad ? new SquadModel(squad) : null), [squad]);
  ...
  const roleOptions = isLeader
    ? SquadModel.memberRoleOptions
    : SquadModel.memberRoleOptions.filter(option => option.value !== SquadRole.SUBLEADER);
  ...
  const handleChangeMemberRole = useCallback(
    async (member: User, role: SquadRole) => {
      try {
        ...
        await squadsApi.updateSquadMemberRole({ userId: member.id, role });
  ...
  const handleTransferLeadership = useCallback(async () => {
    ...
    try {
      ...
      await squadsApi.transferSquadLeadership(leadershipTransferCandidate.id);
  ...
  const handleChangeMemberSpecializations = useCallback(
    async (member: User, specializationIds: string[]) => {
      try {
        ...
        await specializationsApi.setUserSpecializations({ userId: member.id, specializationIds });
      ...
      <div className="flex flex-col gap-2 justify-center">
        ...
        <View.Condition if={hasPendingInvites}>
          <SquadInviteList
            ...
            model={acceptOrRejectInviteState}
  ...
  return (
    <div className="flex flex-col gap-4 rounded-xl border border-white/10 bg-black/60 p-4 shadow-md">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex shrink-0 flex-wrap gap-2 sm:justify-end">
            {canManageSquadMembers && (
              <>
                <Button
                  ...
                  className="w-fit"
                  onClick={() => {
                    inviteToSquadState.visibility.open({ squad });
                  }}>
                  Запросити
                </Button>
                <InviteToSquadModal
            ...
            <Button
              ...
              className="w-fit"
              onClick={() => {
                leaveFromSquadState.visibility.open({
                  squad,
      ...
      <KickFromSquadModal
        model={kickFromSquadState}
        onKickSuccess={() => {
          void refreshSquadState();
        }}
      />

      <LeaveFromSquadModal model={leaveFromSquadState} onLeaveSuccess={refreshSquadState} />
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-2">
                      <span className="rounded-full border border-white/10 bg-black/50 px-2 py-0.5 text-[10px] uppercase tracking-[0.14em] text-zinc-400">
                        {managedMember.id === squad.leader?.id
                          ? 'Лідер'
                          : SquadModel.roleLabels[managedMember.squadRole ?? SquadRole.MEMBER]}
                      </span>
                ...
                <div className="flex flex-wrap w-full gap-2 border-t border-white/10 pt-4">
                  ...
                  {canKickMember(managedMember) && (
                    <Button
                      ...
                      onClick={() => {
                        handleCloseManagedMemberDrawer();
                        kickFromSquadState.visibility.open({ user: managedMember });
                      }}>
                      Вилучити із загону
                    </Button>
                  )}
            ...
            {Array.isArray(squad.members) && squad.members.length > 0 ? (
              <ul className="flex flex-col gap-2">
                {[
                  ...
                  ...(squadModel?.membersSortedByRole ?? []).filter(member => member.id !== squad.leader?.id),
                ].map(member => (
                  <li
                    key={member.id}
                    ...
                    <div className="flex-1 flex items-center gap-2 min-w-0">
                      ...
                      <span className="rounded-full border border-white/10 bg-black/50 px-2 py-0.5 text-[10px] uppercase tracking-[0.14em] text-zinc-400">
                        {member.id === squad.leader?.id
                          ? 'Лідер'
                          : SquadModel.roleLabels[member.squadRole ?? SquadRole.MEMBER]}
                      </span>
