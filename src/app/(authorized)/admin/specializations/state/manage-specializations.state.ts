...
export class ManageSpecializationState {
  constructor() {
  ...
  createSpecialization = async (
    ...
    try {
      this.loader.start();
      const { data } = await specializationsApi.createSpecialization(dto);
  ...
  updateSpecialization = async (
    ...
    try {
      this.loader.start();
      const { data } = await specializationsApi.updateSpecialization(dto);
  ...
  deleteSpecialization = async (id: string, onSuccess?: (specialization: Specialization) => void) => {
    try {
      this.loader.start();
      const { data } = await specializationsApi.deleteSpecialization(id);
