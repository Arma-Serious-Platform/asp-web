...
export class ManageIslandState {
  constructor() {
  ...
  createIsland = async (dto: CreateIslandDto, onSuccess?: (island: Island) => void) => {
    try {
      this.loader.start();
      const { data } = await islandsApi.createIsland(dto);
  ...
  updateIsland = async (id: string, dto: UpdateIslandDto, onSuccess?: (island: Island) => void) => {
    try {
      this.loader.start();
      const { data } = await islandsApi.updateIsland(id, dto);
  ...
  deleteIsland = async (id: string, onSuccess?: () => void) => {
    try {
      this.loader.start();
      await islandsApi.deleteIsland(id);
