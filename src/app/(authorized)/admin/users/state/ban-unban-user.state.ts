...
export class BanUnbanUserState {
  constructor() {
  ...
  async banUser(dto: BanUserDto, onSuccess?: (user: User) => void) {
    try {
      this.loader.start();
      const { data } = await usersApi.banUser(dto);
  ...
  async permanentlyBanUser(userId: string, reason: string, onSuccess?: (user: User) => void) {
    try {
      this.loader.start();
      const { data } = await usersApi.permanentlyBanUser(userId, reason);
  ...
  async unbanUser(userId: string, reason?: string, onSuccess?: (user: User) => void) {
    try {
      this.loader.start();
      const { data } = await usersApi.unbanUser(userId, reason);
...
export const banUnbanUserState = new BanUnbanUserState();
