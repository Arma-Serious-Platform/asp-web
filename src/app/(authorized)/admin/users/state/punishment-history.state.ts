...
export class PunishmentHistoryState {
  constructor() {
  ...
  async load() {
    ...
    try {
      ...
      const [warningsResponse, historyResponse] = await Promise.all([
        usersApi.findUserWarnings(userId),
        usersApi.findUserPunishmentHistory(userId),
      ]);
  ...
  async removeWarning(warningId: string, reason?: string, onSuccess?: (warning: UserWarning) => void) {
    try {
      ...
      const { data } = await usersApi.removeUserWarning(warningId, reason);
...
export const punishmentHistoryState = new PunishmentHistoryState();
